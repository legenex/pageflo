 ...
 */
/*
 * Moved to `lib/relation-id.ts` (a leaf) and re-exported here so the many
 * existing importers keep working. The access layer and collection hooks must
 * import the LEAF, not this module: this file reaches `lib/auth`, which
 * reaches `payload.config`, and that closes a production-only circular-import
 * crash. See relation-id.ts for the incident note.
 */
