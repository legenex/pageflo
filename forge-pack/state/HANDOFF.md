...
- Honest delivery state from a `delivery_log` that is append-only by design and write-locked (concurrent writers can drop an entry); `downstream.completed` is not "delivered"; `leads.delivery_state`.
...
- Production harnesses rewritten (evidence-bearing assertions, exact-record targeting; no clone/master/deployment/domain per run, but two QA leads per run).
