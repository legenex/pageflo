...
| R6.6 | Browser E2E: ≥3 materially different LP templates, same flow: mount region, content survives, advance, branching, progress, backtrack clears state, lead form, consent, exactly one POST, exactly one row, mobile+desktop usable, static buttons inert | `test:e2e` 34 assertions (production build) + `test:ui` public proofs; mobile pass (B-F) pending | IN PROGRESS |
...
| R12.2 | Full suites from the integrated branch (no source-branch counts) | unit 2565 + isolation + identity 33 + release 27 + bootstrap 55 + dom 357 + ui 85+ + e2e 34; counts re-run post-changes | PASS |
...
| R12.4 | Screenshots: LP templates list, LP editor, quiz templates list, quiz editor, quiz deployment General, LP deployment General, public LP w/ runtime, desktop + mobile | docs/screenshots/*.png (desktop set regenerated); mobile shots (B-F) pending | IN PROGRESS |
| R12.5 | Keyboard access, visible focus, labels, contrast, no overflow, no hydration errors, no console errors, no failed product requests | test:ui requirement-H extension (B-F) | IN PROGRESS |
