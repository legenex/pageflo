...
try {
  ...
  for (const ex of expectations) {
    ...
    t(c.method === (ex.key === 'website' ? 'checkbox_disclosure_unverified' : 'checkbox_unchecked_default'), `${ex.label}: and records the method (${c.method}); a block\'s own copy cannot be verified against the Brand text`)
  ...
  const forgedText = await post({ funnel_type: 'quiz', contact: contact('forged'), client_submission_id: `${RUN}-forged`, consent: { accepted: true, disclosure_text: 'a disclosure the visitor never saw <script>x</script>', client_accepted_at: '1999-01-01T00:00:00Z' } })
  t(forgedText.status === 200, 'a forged disclosure is captured but never trusted')
  const forgedLead = await waitFor(async () => (await payload.find({ collection: 'leads', where: { 'contact.email': { equals: `${RUN}-api-forged@example.test` } }, limit: 1, depth: 0, overrideAccess: true })).docs[0] as Record<string, any> | undefined)
  if (forgedLead) track('leads', forgedLead as { id: number })
  t(forgedLead?.consent?.method === 'checkbox_disclosure_unverified' && !String(forgedLead?.consent?.disclosure_text).includes('script') && !forgedLead?.consent?.client_accepted_at, 'and is stored as unverified, without markup, without the forged clock')
